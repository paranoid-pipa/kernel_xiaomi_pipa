...
static void set_cps_config(struct snd_soc_pcm_runtime *rtd, u32 num_ch,
			   u32 ch_mask)
{
	int i = 0;
	int val = 0;
	...
	afe_set_cps_config(msm_get_port_id(dai_link->id), &pdata->cps_config,
			   ch_mask);
}
		...
		list_for_each_entry(aux_comp,
			...
			if (aux_comp->name != NULL && (
				...
				wsa_macro_set_spkr_mode(
					component, WSA_MACRO_SPKR_MODE_DEFAULT);
			}
		}
	}
...
static struct snd_soc_dai_link msm_kona_dai_links
	[ARRAY_SIZE(msm_common_dai_links) + ARRAY_SIZE(msm_bolero_fe_dai_links) +
	 ARRAY_SIZE(msm_common_misc_fe_dai_links) +
	 ARRAY_SIZE(msm_common_be_dai_links) + ARRAY_SIZE(msm_mi2s_be_dai_links) +
	...
	if (!strcmp(match->data, "codec")) {
		...
		if (rc) {
			...
			if (mi2s_audio_intf) {
				...
				if (get_hw_version_platform() ==
					    HARDWARE_PLATFORM_UMI ||
				    get_hw_version_platform() ==
					    HARDWARE_PLATFORM_CMI ||
				    get_hw_version_platform() ==
					    HARDWARE_PLATFORM_APOLLO ||
				    get_hw_version_platform() ==
					    HARDWARE_PLATFORM_ALIOTH ||
				    get_hw_version_platform() ==
					    HARDWARE_PLATFORM_THYME ||
				    get_hw_version_platform() ==
					    HARDWARE_PLATFORM_ENUMA ||
				    get_hw_version_platform() ==
					    HARDWARE_PLATFORM_ELISH ||
				    get_hw_version_platform() ==
					    HARDWARE_PLATFORM_PSYCHE ||
				    get_hw_version_platform() ==
					    HARDWARE_PLATFORM_DAGU ||
				    get_hw_version_platform() ==
					    HARDWARE_PLATFORM_CAS) {
					memcpy(msm_kona_dai_links + total_links,
				...
				} else if (get_hw_version_platform() ==
						   HARDWARE_PLATFORM_LMI ||
					   get_hw_version_platform() ==
						   HARDWARE_PLATFORM_MUNCH) {
					memcpy(msm_kona_dai_links + total_links,
					...
					dev_info(dev, "%s: Using pri_mi2s_rx_tfa9874_dai_links\n", __func__);
					   ...
					   HARDWARE_PLATFORM_PIPA) {
					memcpy(msm_kona_dai_links + total_links,
					       tert_mi2s_rx_aw882xx_dai_links,
					       sizeof(tert_mi2s_rx_aw882xx_dai_links));
					total_links += ARRAY_SIZE(
						tert_mi2s_rx_aw882xx_dai_links);
					dev_info(
						dev,
						"%s: Using tert_mi2s_rx_aw882xx_dai_links\n",
						__func__);
				}
			}
		}
				    ...
				    struct msm_asoc_mach_data *pdata)
{
	...
	ret = of_property_read_u32_array(
		pdev->dev.of_node, "qcom,cps_reg_phy_addr", dt_values,
	...
	ret = of_property_read_u32_array(
		pdev->dev.of_node, "qcom,cps_threshold_levels", dt_values,
	...
	pdata->cps_config.spkr_dep_cfg = devm_kzalloc(
		&pdev->dev,
		sizeof(struct lpass_swr_spkr_dep_cfg_t) * pdata->wsa_max_devs,
		GFP_KERNEL);
	if (!pdata->cps_config.spkr_dep_cfg) {
	...
	ret = of_property_read_u32_array(
		pdev->dev.of_node, "qcom,cps_wsa_vbatt_temp_reg_addr",
		dt_values, sizeof(dt_values) / sizeof(dt_values[0]) - 1);
	if (ret) {
		dev_dbg(&pdev->dev, "%s: could not find %s entry in dt\n",
	...
	ret = of_property_read_u32_array(
		pdev->dev.of_node, "qcom,cps_normal_values", dt_values,
		sizeof(dt_values) / sizeof(dt_values[0]));
	if (ret) {
		dev_dbg(&pdev->dev, "%s: could not find %s entry in dt\n",
	...
	ret = of_property_read_u32_array(
		pdev->dev.of_node, "qcom,cps_lower1_values", dt_values,
		sizeof(dt_values) / sizeof(dt_values[0]));
	if (ret) {
		dev_dbg(&pdev->dev, "%s: could not find %s entry in dt\n",
	...
	ret = of_property_read_u32_array(
		pdev->dev.of_node, "qcom,cps_lower2_values", dt_values,
		sizeof(dt_values) / sizeof(dt_values[0]));
	if (ret) {
		dev_dbg(&pdev->dev, "%s: could not find %s entry in dt\n",
