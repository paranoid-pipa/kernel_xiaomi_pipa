...
static ssize_t wsa881x_swrslave_reg_show(struct swr_device *pdev,
...
static int wsa881x_swr_probe(struct swr_device *pdev)
{
	...
	if (!wsa881x->debugfs_dent) {
		...
		if (!IS_ERR(wsa881x->debugfs_dent)) {
			wsa881x->debugfs_peek = debugfs_create_file(
				"swrslave_peek", S_IFREG | 0444,
				wsa881x->debugfs_dent, (void *)pdev,
				&codec_debug_read_ops);

			wsa881x->debugfs_poke = debugfs_create_file(
				"swrslave_poke", S_IFREG | 0444,
				wsa881x->debugfs_dent, (void *)pdev,
				&codec_debug_write_ops);

			wsa881x->debugfs_reg_dump = debugfs_create_file(
