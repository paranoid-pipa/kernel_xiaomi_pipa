				   ...
				   u8 *conf_levels_ptr)
{
	...
	if (client->num_sound_models != 0) {
		if (client->num_keywords == 0) {
			pr_debug(
				"%s: no number of confidence_values provided\n",
				__func__);
			client->multi_snd_model_confidence_levels = NULL;
			goto done;
		}

		client->multi_snd_model_confidence_levels = kzalloc(
			(sizeof(uint32_t) * client->num_keywords), GFP_KERNEL);
		...
		if (copy_from_user(
			    (u8 *)client->multi_snd_model_confidence_levels,
			    conf_levels_ptr,
		...
		client->confidence_levels = kzalloc(
			(sizeof(uint8_t) * client->num_confidence_levels),
			GFP_KERNEL);
		...
		if (copy_from_user(client->confidence_levels, conf_levels_ptr,
			    ...
			    struct lsm_params_info_v2 *p_info)
{
	...
	if (p_info->param_type == LSM_MULTI_SND_MODEL_CONFIDENCE_LEVELS) {
		...
		rc = q6lsm_set_one_param(
			prtd->lsm_client, p_info,
		...
		if (prtd->lsm_client->multi_snd_model_confidence_levels) {
			kfree(prtd->lsm_client
				      ->multi_snd_model_confidence_levels);
			prtd->lsm_client->multi_snd_model_confidence_levels =
				NULL;
		}
	} else {
		if (p_info->param_size > MAX_NUM_CONFIDENCE) {
			dev_err(rtd->dev, "%s: invalid confidence levels %d\n",
				__func__, p_info->param_size);
			return -EINVAL;
		}

		prtd->lsm_client->num_confidence_levels = p_info->param_size;
		rc = msm_lsm_get_conf_levels(prtd->lsm_client,
					     p_info->param_data);
		if (rc) {
			dev_err(rtd->dev,
			     ...
			     struct lsm_params_info_v2 *p_info)
{
	...
	if (p_info->model_id != 0 &&
		...
		rc = q6lsm_snd_model_buf_alloc(client, p_info->param_size,
					       p_info, sm);
		if (rc) {
			dev_err(rtd->dev,
		 ...
		 */
		if (copy_from_user((u8 *)sm->data + offset, p_info->param_data,
				   p_info->param_size)) {
			dev_err(rtd->dev,
		...
		list_add_tail(&sm->list,
			      &client->stage_cfg[stage_idx].sound_models);
		...
		if (rc) {
			dev_err(rtd->dev,
				"%s: Failed to register snd_model, err = %d\n",
				__func__, rc);
			goto err_copy;
		}
	} else {
			       ...
			       struct lsm_params_info_v2 *p_info)
{
	...
	if (p_info->model_id != 0 &&
		...
		list_for_each_entry (
			sm, &client->stage_cfg[p_info->stage_idx].sound_models,
			list) {
			dev_dbg(rtd->dev,
		...
		struct lsm_params_info_v2 *p_info)
{
	...
	if (p_info->param_type == LSM_REG_MULTI_SND_MODEL &&
	    prtd->lsm_client->num_sound_models ==
		    LSM_MAX_SOUND_MODELS_SUPPORTED) {
		dev_err(rtd->dev,
...
#if defined(CONFIG_MACH_XIAOMI_DAGU)
		rc = q6lsm_snd_model_buf_alloc(
			prtd->lsm_client, snd_model_v2.data_size, &p_info, sm);
#else
		rc = q6lsm_snd_model_buf_alloc(prtd->lsm_client,
...
#if defined(CONFIG_MACH_XIAOMI_DAGU)
				ptr_info_v2->module_id =
					temp_ptr_info->module_id;
				ptr_info_v2->param_id = temp_ptr_info->param_id;
				ptr_info_v2->param_size =
					temp_ptr_info->param_size;
				ptr_info_v2->param_data =
					temp_ptr_info->param_data;
				ptr_info_v2->param_type =
					temp_ptr_info->param_type;
...
static int msm_lsm_close(struct snd_pcm_substream *substream)
{
	...
	if (prtd->lsm_client->lab_enable && prtd->lsm_client->lab_buffer) {
		ret = msm_lsm_lab_buffer_alloc(prtd, LAB_BUFFER_DEALLOC);
		if (ret)
			dev_err(rtd->dev,
				"%s: lab buffer dealloc failed ret %d\n",
				__func__, ret);
