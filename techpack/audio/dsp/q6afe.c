...
static int afe_send_cps_config(int src_port)
{
	...
	param_size =
		sizeof(struct afe_cps_hw_intf_cfg) -
		sizeof(this_afe.cps_config->spkr_dep_cfg) +
		(sizeof(struct lpass_swr_spkr_dep_cfg_t) * this_afe.num_spkrs);
...
static void afe_send_cal_spv4_tx(int port_id)
{
	...
	if ((this_afe.prot_cfg.mode != MSM_SPKR_PROT_DISABLED) &&
		...
		size = sizeof(struct afe_sp_v4_param_th_vi_r0t0_cfg) +
		       (this_afe.num_spkrs *
			sizeof(struct afe_sp_v4_channel_r0t0));
	...
	if ((this_afe.th_ftm_cfg.mode == MSM_SPKR_PROT_IN_FTM_MODE) &&
		...
		size = sizeof(struct afe_sp_v4_param_th_vi_ftm_cfg) +
		       (this_afe.num_spkrs *
			sizeof(struct afe_sp_v4_channel_ftm_cfg));
		tmp_ptr = kzalloc(size, GFP_KERNEL);
		if (!tmp_ptr) {
			mutex_unlock(
			    ...
			    struct afe_ttp_config *ttp_cfg)
{
	...
	if ((this_afe.cps_config) && (this_afe.vi_rx_port == port_id)) {
...
int aw_send_afe_rx_module_enable(void *buf, int size)
{
	...
	if (afe_spk_prot_prepare(port_id, 0, AFE_PARAM_ID_AWDSP_RX_SET_ENABLE,
...
int aw_send_afe_tx_module_enable(void *buf, int size)
{
	...
	if (afe_spk_prot_prepare(port_id, 0, AFE_PARAM_ID_AWDSP_TX_SET_ENABLE,
				 &config, sizeof(union afe_spkr_prot_config))) {
		pr_err("%s: set bypass failed \n", __func__);
		return -EINVAL;
	}
...
int aw_send_afe_cal_apr(uint32_t param_id, void *buf, int cmd_size, bool write)
{
	...
	if (write) {
		...
		q6common_pack_pp_params(aw_cal->cal_data.kvaddr, &param_hdr,
					buf, &payload_size);
	...
	/*Send/Get package to/from ADSP*/
	mem_hdr.data_payload_addr_lsw = lower_32_bits(aw_cal->cal_data.paddr);
	mem_hdr.data_payload_addr_msw =
		msm_audio_populate_upper_32_bits(aw_cal->cal_data.paddr);
	mem_hdr.mem_map_handle = aw_cal->map_data.map_handle;

	pr_debug(
		"%s: Sending aw_cal port = 0x%x, cal size = %zd, cal addr = 0x%pK\n",
		__func__, port_id, aw_cal->cal_data.size,
...
void aw_cal_unmap_memory(void)
{
	...
	if (this_afe.aw_cal.map_data.map_handle) {
		result = afe_unmap_rtac_block(
			&this_afe.aw_cal.map_data.map_handle);
 ...
 *
 */
void afe_set_cps_config(int src_port, struct afe_cps_hw_intf_cfg *cps_config,
			u32 ch_mask)
{
