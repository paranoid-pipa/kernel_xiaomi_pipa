 ...
 */
int adm_apr_send_pkt(void *data, wait_queue_head_t *wait, int port_idx,
		     int copp_idx, int opcode)
{
	int ret = 0;
	...
	if (ret > 0) {
		ret = wait_event_timeout(*wait, atomic_read(copp_stat) >= 0,
					 time_out);
	...
	if (data->payload_size >= sizeof(uint32_t)) {
		...
		if (data->opcode == APR_BASIC_RSP_RESULT) {
			...
			case ADM_CMD_SET_PP_PARAMS_V6:
				...
				else if (rtac_make_adm_callback(
			...
			case ADM_CMD_DEVICE_OPEN_V8:
				pr_debug(
					"%s: Basic callback received for 0x%x, wake up.\n",
					__func__, payload[0]);
