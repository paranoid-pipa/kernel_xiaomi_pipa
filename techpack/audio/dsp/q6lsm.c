			     ...
			     struct lsm_sound_model *sm)
{
	...
	ret = q6lsm_pack_params(sm->data, &param_hdr, NULL, offset,
					...
					uint32_t model_id)
{
	...
	struct lsm_param_multi_snd_model_conf_levels *multi_sm_conf_levels =
		NULL;
	...
	if (model_id != 0) {
		...
		memcpy(conf_levels->confidence_levels,
		       client->confidence_levels, num_conf_levels);
...
int q6lsm_deregister_sound_model(struct lsm_client *client)
{
	...
	if (client->num_sound_models > 0) {
		...
		list_for_each_entry_safe (
			...
			list) {
			pr_debug(
				"%s: current snd_model: %d, num of sound models left %d\n",
				__func__, sm->model_id,
		...
		if (sm && sm->data) {
			...
			rc = q6lsm_apr_send_pkt(client, client->apr, &cmd.hdr,
						true, NULL);
			if (rc) {
				...
				pr_debug(
					"%s: Deregister sound model succeeded\n",
					__func__);
			}
			      ...
			      struct lsm_sound_model *sm)
{
	...
	rc = q6lsm_memory_map_regions(client, sm->phys, len,
				      &sm->mem_map_handle);
	if (rc) {
			...
			uint32_t param_type)
{
	...
	case LSM_MULTI_SND_MODEL_CONFIDENCE_LEVELS:
		...
		param_info.param_id = p_info->param_id;
#if defined(CONFIG_MACH_XIAOMI_DAGU)
		rc = q6lsm_send_confidence_levels(client, &param_info,
						  LSM_SESSION_CMD_SET_PARAMS_V2,
						  p_info->model_id);
		if (rc)
...
#endif
	{
		...
		if (param_type == LSM_REG_MULTI_SND_MODEL) {
			list_for_each_entry (
				sm,
				&client->stage_cfg[p_info->stage_idx]
					 .sound_models,
				list) {
				pr_debug(
...
#endif
	{
		...
		if (param_type == LSM_DEREG_MULTI_SND_MODEL) {
			...
			rc = q6lsm_pack_and_set_params(
				client, &param_info,
				(uint8_t *)&p_info->model_id,
				LSM_SESSION_CMD_SET_PARAMS_V2);
		} else {
			rc = q6lsm_pack_and_set_params(
				client, &param_info, NULL,
				LSM_SESSION_CMD_SET_PARAMS_V2);
		}
		if (rc)
			pr_err("%s: %s failed, rc %d\n", __func__,
